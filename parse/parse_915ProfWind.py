...
def uploadCSV(csv_data):
    ...
    for row in csv_data:
        ...
        if line_num == 1:
            ...
            insert_measurement += """VALUES ('{}',{},'{}',{})""".format(
                fileLocation, wind_profiler_instrument['asset_id'], wind_profiler_instrument['measurement_date_time'], ", ".join(values))
            ...
            insert_gate_response = """INSERT INTO WindProfilerGateResponse({},""".format(
                ", ".join(gate_response_columns))
            for i in range(1, num_radials + 1):
                gate_response_radial_columns.append("`VEL-{}`".format(str(i)))
                gate_response_radial_columns.append(
                    "`VEL-{}_QCFLAG`".format(str(i)))
                gate_response_radial_columns.append("`OBS-{}`".format(str(i)))
                gate_response_radial_columns.append(
                    "`OBS-{}_QCFLAG`".format(str(i)))
                gate_response_radial_columns.append("`SNR-{}`".format(str(i)))
                gate_response_radial_columns.append(
                    "`SNR-{}_QCFLAG`".format(str(i)))
            insert_gate_response += """{})""".format(
                ", ".join(gate_response_radial_columns))
            # items in row: 8 + (6 columns per radial) * num radials
            insert_gate_response += """VALUES ({}, {})""".format(
                str(measurementID), ", ".join(row[1: 8 + num_radials * 6]))
